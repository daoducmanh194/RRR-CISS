...
from collections.abc import Sequence  # import for py 3.10
import os
...
class _BaseWrapper(object):
    ...
    def _encode_one_hot(self, ids):
        ...
        # rows = torch.arange(0, one_hot.size(0))[:,None]
        # n_col = ids.size(1)
        # one_hot[rows.repeat(1, n_col), ids] = 1
...
class BackPropagation(_BaseWrapper):
    ...
    def __call__(self, inputs, model, task_id):
        ...
        ids_ = torch.LongTensor([ids[:, 0].tolist()]).T.to(device=device)
        self.backward(ids_)
...
class GradCAM(_BaseWrapper):
    ...
    #     self.args = args
        ...
        self.target_layer = args.target_layer  # string of layer name

        self.upsample = args.rrr_upsample
    ...
    def __call__(self, inputs, model, task_id):
        ...
        ids_ = torch.LongTensor([ids[:, 0].tolist()]).to(device=device)
        self.backward(ids_)
        # print("DO HERE")
...
class SmoothGrad(_BaseWrapper):
    ...
    def __call__(self, inputs, model, task_id):
        ...
        if self.args.rrr_method == 'gsmooth':
            def backward_hook(module, grad_in, grad_out):
                # Cut off negative gradients
                if isinstance(module, nn.ReLU):
                    return (F.relu(grad_in[0]),)
