...
def make_directories(args):
     ...
     head = 'mh'
     name = '{}_{}_{}_{}_budget{}'.format(args.dataset,
                                       'rrr',
                                       args.backbone,
                                       head,
                                       args.mem_size)
     if args.xai_memory:
         name = '{}_{}_{}_{}_{}_budget{}'.format(args.dataset,
                                              'rrr',
                                              args.backbone,
                                              head,
                                              args.rrr_method,
     ...
     if args.wandb_notes == '':
         args.wandb_notes = name
     else:
        args.wandb_notes = args.wandb_notes + '_' + name


     if not os.path.exists(args.path_checkpoint):
         os.mkdir(args.path_checkpoint)

     args.path_checkpoint = os.path.join(args.path_checkpoint,args.wandb_notes)
     if not os.path.exists(args.path_checkpoint):
         os.mkdir(args.path_checkpoint)

     return args.path_checkpoint, args.wandb_notes
...
def save_code(args):
    cwd = os.getcwd()
    des = os.path.join(args.path_checkpoint, 'code') + '/'
