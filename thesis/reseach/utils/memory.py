...
def memory_sampling_balanced(args, prev_model):
    ...
    train_dst = dataset(args=args, image_set='train', transform=transform, cil_step=args.task_id-1)
    ...
    memory_json = f'/kaggle/working/RRR-CISS/thesis/reseach/memory.json'
    
    if args.task_id > 1:
        with open(memory_json, "r") as json_file:
            memory_list = json.load(json_file)

        memory_candidates = memory_list[f"step_{args.task_id-1}"]["memory_candidates"]
    else:
        memory_list = {}
        memory_candidates = []
    ...
    for images, targets, _, img_names in train_loader:
        ...
        for b in range(images.size(0)):
            ...
            labels = (labels - 1).tolist() if args.unknown else labels.tolist()
    ...
    while args.mem_size > num_sampled:
    ...
    memory_list[f"step_{args.task_id}"] = {"memory_candidates": sampled_memory_list, 
